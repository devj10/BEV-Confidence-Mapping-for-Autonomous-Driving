...
def train_from_azure_tmp():
    ...
    checkpoint_src = Path("/root/cs231n/results/checkpoints/model_final.pt")
    checkpoint_dst = Path("/root/outputs/model_final.pt")

    if not checkpoint_src.exists():
        raise FileNotFoundError(f"Missing checkpoint at {checkpoint_src}")
